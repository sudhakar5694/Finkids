angular
    ...
    function (
      ...
      ($scope.dynamicStats = [
        ...
        },
        {
          ...
          count: "0",
          chart_data: ["64/100"],
          chart_options: { height: 24, width: 24, fill: ["#8bc34a", "#eee"] },
        },
        {
          id: "4",
          title: "Visitors (live)",
          ...
          ($scope.dynamicStats[0].count = "12456"),
            ...
            $interval(function () {
              var random = Math.round(10 * Math.random()),
                live_values = $scope.dynamicStats[3].chart_data
                  .toString()
                  .split(",");
              live_values.shift(),
              ...
              ($scope.dynamicStats[3].chart_data = live_values),
                ($scope.dynamicStats[3].count =
                  $scope.dynamicStats[3].count == countTo
                    ? countTo
                    : getRandomVal(20, 120));
            }, 2e3);
        }),
      ...
      for (var i = 0; i < $scope.sale_chart_data.length; i++)
        $scope.sale_chart_data[i] = MG.convert.date(
          $scope.sale_chart_data[i],
          "date"
        );
      ($scope.sale_chart_options = {
        ...
        y_accessor: "value",
        markers: [
          { date: new Date("2016-02-26T00:00:00.000Z"), label: "Winter Sale" },
        ...
        ($scope.chartist_stat_options_responsive = [
          ...
          [
            ...
            { stackBars: !1, seriesBarDistance: 10 },
          ],
          [
        ...
        ($scope.video_data = [
          { id: "-CYs99M7hzA", name: "Unboxing the HERO4", source: "Mashable" },
          {
            id: "te689fEo2pY",
      ...
      ($scope.videoChange = function ($event, videoID) {
        ...
        if (!$this.hasClass("md-list-item-active")) {
          var iframe_embed =
            '<iframe height="150" width="300" data-uk-cover src="https://www.youtube.com/embed/' +
            videoID +
            '?rel=0" frameborder="0" allowfullscreen style="max-height:100%"></iframe>';
          $video_playlist.children("li").removeClass("md-list-item-active"),
            $this.addClass("md-list-item-active"),
            $video_player.velocity(
              { translateZ: 0, scale: 0, opacity: 0 },
              {
                duration: 280,
      ...
      function animate_change() {
        $clndr_events
          ...
          $timeout(function () {
            $clndr_events.children(".clndr").removeClass("animated_change");
          }, 380);
      }
      ...
      ($scope.clndr_options = { weekOffset: 1, daysOfTheWeek: daysOfTheWeek }),
        ...
        ($scope.showEvents = function ($event, day) {
          ...
          if (day.events.length && !$this.hasClass("day-active")) {
            var delay = $clndr_events.hasClass("events_visible") ? 0 : 280;
            $clndr_events.addClass("events_visible"),
              $scope.clndrWidth(),
              ($scope.selectedDay = function (event) {
                return (
                  moment(day.date).format("M-D-YYYY") ==
                  moment(event.date).format("M-D-YYYY")
                );
              }),
              $timeout(function () {
                $(".clndr_events")
                  .children(".clndr_event")
                  .velocity("transition.slideUpIn", { stagger: 100, drag: !0 });
              }, delay),
              $this
                .siblings(".day")
        ...
        ($scope.addEvent = function ($event) {
          ...
          if (!$scope.newEvent.title)
            return $(e_title).addClass("md-input-danger").focus().change(), !1;
          if (!$scope.newEvent.date)
            return $(e_date).addClass("md-input-danger").focus().change(), !1;
          var new_event = [
            {
              date: $(e_date).val(),
              title: $(e_title).val(),
        ...
        ($scope.clndrWidth = function () {
          var calMinWidth =
            7 * $clndr_events.find(".day > span").outerWidth() + 240 + 32 + 14;
          $clndr_events.width() < calMinWidth
            ? $clndr_events.addClass("events_over")
            : $clndr_events.removeClass("events_over");
        }),
        ($scope.user_data = user_data[0]),
        ($scope.sSidebar = { site_online: !0, top_bar: !0, minify_assets: !0 }),
        ($scope.sSidebar_users = [
          { name: "Catalina Weber", avatar: "avatar_02" },
          { name: "Jayme Hermiston", avatar: "avatar_03" },
          { name: "Enid Roob", avatar: "avatar_06" },
          { name: "Lelah Leffler", avatar: "avatar_04" },
        ]);
    },
  ])
  .controller("userMapsCtrl", [
    ...
    function ($rootScope, $scope, $timeout, utils, NgMap) {
      ($scope.marker_icon = {
        url: utils.isHighDensity()
          ? "assets/img/md-images/ic_location_history_black_48dp.png"
          : "assets/img/md-images/ic_location_history_black_24dp.png",
        ...
        ($scope.showMarker = function (evt, id) {
          ($scope.user = $scope.gmap_users[id]),
            $scope.showInfoWindow.apply(this, [
              evt,
              "infoWindow",
